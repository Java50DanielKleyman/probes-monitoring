...
public class AnalyzerAppl {
	...
	private void probeDataAnalyzing(ProbeData probeData) {
		...
		// RangeProviderClientService
		// create a proper deviation and streamBridge.send(producerBindingName,
		// deviation);
		Range range = clientService.getRange(probeData.id());

		if (probeData.value() > range.maxValue() || probeData.value() < range.minValue()) {

			double deviation = probeData.value() > range.maxValue() ? probeData.value() - range.maxValue()
					: probeData.value() - range.minValue();
			streamBridge.send(producerBindingName,
					new DeviationData(probeData.id(), deviation, probeData.value(), System.currentTimeMillis()));
		}
	}
}
